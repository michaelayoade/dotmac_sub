...
def upgrade() -> None:
    ...
    account_columns = {
        col["name"] for col in inspector.get_columns("subscriber_accounts")
    }

    if "splynx_tariff_id" not in offer_columns:
        op.add_column(
            "catalog_offers", sa.Column("splynx_tariff_id", sa.Integer(), nullable=True)
        )
    if "splynx_service_name" not in offer_columns:
        op.add_column(
            "catalog_offers",
            sa.Column("splynx_service_name", sa.String(length=160), nullable=True),
        )
    if "splynx_tax_id" not in offer_columns:
        op.add_column(
            "catalog_offers", sa.Column("splynx_tax_id", sa.Integer(), nullable=True)
        )
    ...
    if "vat_percent" not in offer_columns:
        op.add_column(
            "catalog_offers", sa.Column("vat_percent", sa.Numeric(5, 2), nullable=True)
        )
    if "speed_download_mbps" not in offer_columns:
        op.add_column(
            "catalog_offers",
            sa.Column("speed_download_mbps", sa.Integer(), nullable=True),
        )
    if "speed_upload_mbps" not in offer_columns:
        op.add_column(
            "catalog_offers",
            sa.Column("speed_upload_mbps", sa.Integer(), nullable=True),
        )
    if "aggregation" not in offer_columns:
        op.add_column(
            "catalog_offers", sa.Column("aggregation", sa.Integer(), nullable=True)
        )
    if "priority" not in offer_columns:
        op.add_column(
            "catalog_offers", sa.Column("priority", sa.String(length=40), nullable=True)
        )
    ...
    if "splynx_service_id" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("splynx_service_id", sa.Integer(), nullable=True)
        )
    if "router_id" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("router_id", sa.Integer(), nullable=True)
        )
    if "service_description" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("service_description", sa.Text(), nullable=True)
        )
    if "quantity" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("quantity", sa.Integer(), nullable=True)
        )
    if "unit" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("unit", sa.String(length=40), nullable=True)
        )
    if "unit_price" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("unit_price", sa.Numeric(12, 2), nullable=True)
        )
    ...
    if "discount_type" not in subscription_columns:
        op.add_column(
            "subscriptions",
            sa.Column("discount_type", sa.String(length=40), nullable=True),
        )
    if "service_status_raw" not in subscription_columns:
        op.add_column(
            "subscriptions",
            sa.Column("service_status_raw", sa.String(length=40), nullable=True),
        )
    if "login" not in subscription_columns:
        op.add_column(
            "subscriptions", sa.Column("login", sa.String(length=120), nullable=True)
        )
    if "ipv4_address" not in subscription_columns:
        op.add_column(
            "subscriptions",
            sa.Column("ipv4_address", sa.String(length=64), nullable=True),
        )
    ...
    if "billing_person" not in account_columns:
        op.add_column(
            "subscriber_accounts",
            sa.Column("billing_person", sa.String(length=160), nullable=True),
        )
    if "billing_street_1" not in account_columns:
        op.add_column(
            "subscriber_accounts",
            sa.Column("billing_street_1", sa.String(length=160), nullable=True),
        )
    ...
    if "billing_due" not in account_columns:
        op.add_column(
            "subscriber_accounts", sa.Column("billing_due", sa.Integer(), nullable=True)
        )
    ...
    if "month_price" not in account_columns:
        op.add_column(
            "subscriber_accounts",
            sa.Column("month_price", sa.Numeric(12, 2), nullable=True),
        )
