...
def upgrade() -> None:
    ...
    if "vpn_servers" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "protocol",
                postgresql.ENUM("udp", "tcp", name="vpnprotocol", create_type=False),
                nullable=False,
                server_default="udp",
            ),
            ...
            sa.Column(
                "vpn_network", sa.String(64), nullable=False, server_default="10.8.0.0"
            ),
            sa.Column(
                "vpn_netmask",
                sa.String(64),
                nullable=False,
                server_default="255.255.255.0",
            ),
            # Encryption settings
            sa.Column(
                "cipher",
            ...
            sa.Column(
                "tls_version_min", sa.String(16), nullable=False, server_default="1.2"
            ),
            # TLS authentication
            sa.Column("tls_auth_key", sa.Text(), nullable=True),
            sa.Column(
                "tls_auth_direction", sa.Integer(), nullable=False, server_default="0"
            ),
            ...
            # Connection settings
            sa.Column(
                "keepalive_interval", sa.Integer(), nullable=False, server_default="10"
            ),
            sa.Column(
                "keepalive_timeout", sa.Integer(), nullable=False, server_default="120"
            ),
            sa.Column(
                "max_clients", sa.Integer(), nullable=False, server_default="100"
            ),
            sa.Column(
                "client_to_client", sa.Boolean(), nullable=False, server_default="false"
            ),
            # Routes and DNS
    ...
    if "vpn_clients" in existing_tables:
        existing_constraints = {
            c["name"] for c in inspector.get_unique_constraints("vpn_clients")
        }
    ...
    if "vpn_clients" in existing_tables:
        ...
        if "ix_vpn_clients_nas_device_id" not in existing_indexes:
            op.create_index(
                "ix_vpn_clients_nas_device_id", "vpn_clients", ["nas_device_id"]
            )

    if "vpn_connection_logs" in existing_tables:
        existing_indexes = {
            idx["name"] for idx in inspector.get_indexes("vpn_connection_logs")
        }
        if "ix_vpn_connection_logs_client_id" not in existing_indexes:
            op.create_index(
                "ix_vpn_connection_logs_client_id", "vpn_connection_logs", ["client_id"]
            )
        if "ix_vpn_connection_logs_connected_at" not in existing_indexes:
            op.create_index(
                "ix_vpn_connection_logs_connected_at",
                "vpn_connection_logs",
                ["connected_at"],
            )
