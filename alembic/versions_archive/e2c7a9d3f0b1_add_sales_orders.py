...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "person_id", UUID(as_uuid=True), sa.ForeignKey("people.id"), nullable=False
        ),
        sa.Column(
            "account_id", UUID(as_uuid=True), sa.ForeignKey("subscriber_accounts.id")
        ),
        sa.Column("invoice_id", UUID(as_uuid=True), sa.ForeignKey("invoices.id")),
        sa.Column(
            "service_order_id", UUID(as_uuid=True), sa.ForeignKey("service_orders.id")
        ),
    ...
    op.create_index(
        "ix_sales_orders_payment_status", "sales_orders", ["payment_status"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "inventory_item_id", UUID(as_uuid=True), sa.ForeignKey("inventory_items.id")
        ),
        sa.Column("description", sa.String(255), nullable=False),
        sa.Column(
            "quantity", sa.Numeric(12, 3), nullable=False, server_default="1.000"
        ),
    ...
    op.create_index(
        "ix_sales_order_lines_sales_order_id", "sales_order_lines", ["sales_order_id"]
    )
