...
def upgrade() -> None:
    ...
    connectiontype = postgresql.ENUM(
        "pppoe",
        "dhcp",
        "ipoe",
        "static",
    ...
    provisioningaction = postgresql.ENUM(
        "create_user",
        "delete_user",
        "suspend_user",
        "unsuspend_user",
    ...
    configbackupmethod = postgresql.ENUM(
        "ssh",
        "api",
        "tftp",
        "ftp",
        "snmp",
        name="configbackupmethod",
        create_type=False,
    )
    configbackupmethod.create(op.get_bind(), checkfirst=True)

    nasdevicestatus = postgresql.ENUM(
        "active",
        "maintenance",
        "offline",
        "decommissioned",
        name="nasdevicestatus",
        create_type=False,
    )
    ...
    _add_nas_col(
        "rack_position", sa.Column("rack_position", sa.String(length=40), nullable=True)
    )
    _add_nas_col(
        "management_ip", sa.Column("management_ip", sa.String(length=64), nullable=True)
    )
    _add_nas_col(
        "management_port", sa.Column("management_port", sa.Integer(), nullable=True)
    )
    ...
    _add_nas_col(
        "ssh_username", sa.Column("ssh_username", sa.String(length=120), nullable=True)
    )
    _add_nas_col(
        "ssh_password", sa.Column("ssh_password", sa.String(length=255), nullable=True)
    )
    _add_nas_col("ssh_key", sa.Column("ssh_key", sa.Text(), nullable=True))
    _add_nas_col(
        "api_username", sa.Column("api_username", sa.String(length=120), nullable=True)
    )
    _add_nas_col(
        "api_password", sa.Column("api_password", sa.String(length=255), nullable=True)
    )
    ...
    _add_nas_col(
        "snmp_version", sa.Column("snmp_version", sa.String(length=10), nullable=True)
    )
    ...
    _add_nas_col(
        "backup_enabled",
        sa.Column(
            "backup_enabled", sa.Boolean(), server_default="false", nullable=False
        ),
    )
    _add_nas_col(
        "backup_method",
    ...
    _add_nas_col(
        "backup_schedule",
        sa.Column("backup_schedule", sa.String(length=60), nullable=True),
    )
    _add_nas_col(
        "last_backup_at",
    ...
    _add_nas_col(
        "network_device_id", sa.Column("network_device_id", sa.UUID(), nullable=True)
    )
    op.alter_column(
        "nas_devices",
        "name",
    ...
    if duplicates:
        raise RuntimeError(
            "Duplicate nas_devices.code values detected; resolve before migration."
        )
    nas_uniques = {
        tuple(u["column_names"])
        for u in inspector.get_unique_constraints("nas_devices")
    }
    if ("code",) not in nas_uniques:
        op.create_unique_constraint(None, "nas_devices", ["code"])
    nas_fks = {
        tuple(fk["constrained_columns"])
        for fk in inspector.get_foreign_keys("nas_devices")
    }
    ...
    if ("network_device_id",) not in nas_fks:
        op.create_foreign_key(
            None, "nas_devices", "network_devices", ["network_device_id"], ["id"]
        )
    ...
    _add_radius_col(
        "download_speed", sa.Column("download_speed", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "upload_speed", sa.Column("upload_speed", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "burst_download", sa.Column("burst_download", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "burst_upload", sa.Column("burst_upload", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "burst_threshold", sa.Column("burst_threshold", sa.Integer(), nullable=True)
    )
    ...
    _add_radius_col(
        "inner_vlan_id", sa.Column("inner_vlan_id", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "ip_pool_name", sa.Column("ip_pool_name", sa.String(length=120), nullable=True)
    )
    _add_radius_col(
        "ipv6_pool_name",
        sa.Column("ipv6_pool_name", sa.String(length=120), nullable=True),
    )
    _add_radius_col(
        "session_timeout", sa.Column("session_timeout", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "idle_timeout", sa.Column("idle_timeout", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "simultaneous_use", sa.Column("simultaneous_use", sa.Integer(), nullable=True)
    )
    _add_radius_col(
        "mikrotik_rate_limit",
        sa.Column("mikrotik_rate_limit", sa.String(length=255), nullable=True),
    )
    _add_radius_col(
        "mikrotik_address_list",
        sa.Column("mikrotik_address_list", sa.String(length=120), nullable=True),
    )
    op.alter_column(
        "radius_profiles",
        "name",
    ...
    if duplicates:
        raise RuntimeError(
            "Duplicate radius_profiles.code values detected; resolve before migration."
        )
    radius_uniques = {
        tuple(u["column_names"])
        for u in inspector.get_unique_constraints("radius_profiles")
    }
