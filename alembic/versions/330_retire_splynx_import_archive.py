"""Retire only the proven-empty Splynx import residue.

Revision ID: 330_retire_splynx_import_archive
Revises: 329_subscriber_field_verifications

Splynx was the pre-migration BSS and has no write path into this system. Current
production evidence proves that the archived ticket tables and generic ID
mapping table are populated, so they remain as historical migration evidence
without restoring their retired runtime models. Only the archived quote tables
and obsolete portal-onboarding state are approved for removal.
...
``Subscriber.splynx_customer_id`` and ``splynx_billing_transactions`` remain
separate, previously approved carve-outs.
"""
...
# safe drop order.
#
# These populated tables retain historical support and migration evidence.
# Their runtime models and writers stay retired; physical deletion requires a
# separate reviewed retention decision.
_PRESERVED_TABLES = (
...
# Child tables first: FKs point child -> parent, so this order is also the safe
# drop order. Production aggregates verified these three tables are empty on
# 2026-07-19.
_RETIRED_TABLES = (
...
def _assert_safe_cutover(bind) -> None:
    ...
    The approved retirement targets are empty in production. This gate exists
    for every environment and for the possibility that the fact changes before
    deploy. A non-empty target means the retirement decision must be reopened;
    populated preserved tables are deliberately outside this gate.
    """
...
def downgrade() -> None:
    ...
    A faithful rollback: the three retired tables held no rows when dropped,
    so there is nothing to restore. Preserved evidence tables are never touched.
    """
    op.create_table(
        "splynx_archived_quotes",
