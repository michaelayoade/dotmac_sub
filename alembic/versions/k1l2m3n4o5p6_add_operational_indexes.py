...
def _ix(conn, name: str, ddl: str) -> None:
...
def upgrade() -> None:
    ...
    _ix(conn, "ix_audit_events_occurred_at_desc",
    ...
    _ix(conn, "ix_audit_events_active_occurred",
    ...
    _ix(conn, "ix_radius_sessions_subscriber",
        'CREATE INDEX ix_radius_sessions_subscriber ON radius_active_sessions (subscriber_id)')

    _ix(conn, "ix_radius_sessions_nas",
        'CREATE INDEX ix_radius_sessions_nas ON radius_active_sessions (nas_device_id)')

    _ix(conn, "ix_radius_sessions_start",
        'CREATE INDEX ix_radius_sessions_start ON radius_active_sessions (session_start DESC)')

    # Stale-session cleanup filters by last_update.
    _ix(conn, "ix_radius_sessions_last_update",
        'CREATE INDEX ix_radius_sessions_last_update ON radius_active_sessions (last_update)')

    # ── sessions (auth) ──────────────────────────────────────────────
    # Active-session lookups for a principal.
    _ix(conn, "ix_sessions_subscriber_active",
        'CREATE INDEX ix_sessions_subscriber_active ON sessions (subscriber_id, status, revoked_at)')

    _ix(conn, "ix_sessions_system_user_active",
