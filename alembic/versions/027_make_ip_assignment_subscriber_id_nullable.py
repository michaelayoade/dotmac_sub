"""Make ip_assignments.subscriber_id nullable with ON DELETE SET NULL.

Part of DCP-9 (OLT/ONT standalone decoupling): IP assignments can be managed
as inventory without a subscriber owner. The subscriber_id column becomes
nullable and its foreign key is recreated with ``ON DELETE SET NULL`` so that
subscriber removal no longer cascades into IP assignment loss.

Revision ID: 027_ip_subscriber_nullable
Revises: 026_cpe_subscriber_nullable
...
revision = "027_ip_subscriber_nullable"
down_revision = "026_cpe_subscriber_nullable"
branch_labels = None
depends_on = None


_TABLE = "ip_assignments"
...
def downgrade() -> None:
    """Revert ip_assignments.subscriber_id to NOT NULL without ON DELETE SET NULL.
    ...
    orphan IP assignment rows manually before running the downgrade.
    """
    conn = op.get_bind()
    inspector = sa.inspect(conn)
