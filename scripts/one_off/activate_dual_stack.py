...
3. Assign IPv6 prefixes to active subscribers
4. Summary report
...
def generate_ipv6_prefixes(db: Session, dry_run: bool) -> dict[str, int]:
...
def set_ipv6_on_radius_profiles(db: Session, dry_run: bool) -> dict[str, int]:
...
def main():
    ...
    try:
        ...
        logger.info("=== Step 3: Assign IPv6 to active subscribers ===")
        r3 = assign_ipv6_to_subscribers(db, args.dry_run)
        logger.info("Result: %s\n", r3)

        logger.info("=== COMPLETE ===")
        logger.info("IPv6 prefixes generated: %d", r1["created"])
        logger.info("RADIUS profiles updated: %d", r2["updated"])
        logger.info("Subscribers assigned IPv6: %d", r3["assigned"])
    finally:
        db.close()
