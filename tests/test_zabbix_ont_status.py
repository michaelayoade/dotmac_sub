...
def _ont(ont_id: str, external_id: str):
    return SimpleNamespace(id=ont_id, external_id=external_id)
...
def test_snapshot_marks_status_code_one_online(monkeypatch) -> None:
    ...
    snapshot = zabbix_ont_status.get_olt_ont_snapshot_from_zabbix(
        ...
        client=_FakeZabbixClient(
            [{"key_": "ont.status.walk", "lastvalue": walk_output, "lastclock": "1714564800"}]
        ),
    )
...
def test_snapshot_uses_valid_rx_signal_as_online(monkeypatch) -> None:
    ...
    snapshot = zabbix_ont_status.get_olt_ont_snapshot_from_zabbix(
        ...
        [_ont("ont-1", "0/1/0.5")],
        client=_FakeZabbixClient(
            [{"key_": "opt.rx.walk", "lastvalue": walk_output, "lastclock": "1714564800"}]
        ),
    )
