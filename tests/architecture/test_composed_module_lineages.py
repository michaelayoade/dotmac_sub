...
`pyproject.toml`, a lineage listed in `app/migration_lineages.py`, and a
prerequisite binding in `app/migration_bindings.py`. Each is load-bearing and each fails
...
def _declared_lineages() -> tuple[str, ...]:
    """The composed lineages, from their one owner.

    Previously parsed out of `alembic/env.py` because importing that file needs
    a database URL. The list now lives in `app/migration_lineages.py`, which
    imports nothing heavier than `importlib` and can simply be read.
    """
