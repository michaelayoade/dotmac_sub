...
def test_payment_arrangement_templates_include_currency_with_amounts():
    ...
    assert 'arrangement_currency ~ " " ~ "{:,.2f}".format(arrangement.total_amount)' in detail
    assert 'arrangement_currency ~ " " ~ "{:,.2f}".format(arrangement.installment_amount)' in detail
    ...
    assert '{{ arrangement_currency }} {{ "{:,.2f}".format(arr.total_amount) }}' in listing
