...
@pytest.mark.asyncio
async def test_hour_integrates_bandwidth_samples(db_session, subscriber, subscription):
    base = datetime(2026, 6, 1, 12, 0, tzinfo=UTC)
    for offset in (0, 60):
        db_session.add(
            BandwidthSample(
    ...
    out = await svc.get_usage_summary(db_session, str(subscriber.id), "hour", now=now)
...
@pytest.mark.asyncio
async def test_all_sums_session_octets_including_active(
    ...
    out = await svc.get_usage_summary(db_session, str(subscriber.id), "all", now=now)
...
@pytest.mark.asyncio
async def test_cycle_uses_rated_quota_bucket(db_session, subscriber, subscription):
    now = datetime(2026, 6, 1, 12, 0, tzinfo=UTC)
    db_session.add(
        QuotaBucket(
            subscription_id=subscription.id,
    ...
    out = await svc.get_usage_summary(db_session, str(subscriber.id), "cycle", now=now)
...
@pytest.mark.asyncio
async def test_window_with_no_data_falls_back_without_false_zero(
    db_session, subscriber, subscription
):
    now = datetime(2026, 6, 1, 12, 0, tzinfo=UTC)
    ...
    out = await svc.get_usage_summary(db_session, str(subscriber.id), "today", now=now)
    assert out["series"] == []
    assert out["total_source"] == "sessions"
...
@pytest.mark.asyncio
async def test_usage_summary_403_for_non_subscriber():
    principal = {"principal_type": "system_user", "subscriber_id": str(uuid.uuid4())}
    with pytest.raises(HTTPException) as exc:
        await me_api.my_usage_summary(period="today", db=None, principal=principal)
    assert exc.value.status_code == 403


@pytest.mark.asyncio
async def test_usage_summary_scopes_to_caller(monkeypatch):
    ...
    await me_api.my_usage_summary(period="week", db=None, principal=principal)
