...
def test_migrated_web_slices_do_not_keep_local_semantic_color_maps() -> None:
    ...
    assert '"tones": ["positive", "negative"]' in monitoring_service
    assert '"colors": ["#10b981"' not in monitoring_service
    assert "tone=device_kpis.working.tone" in device_list
    assert "tone=device_kpis.not_working.tone" in device_list
