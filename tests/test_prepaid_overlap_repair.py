...
def _misc_invoice_line(db, invoice, *, amount=Decimal("50.00")):
    db.add(
        InvoiceLine(
            invoice_id=invoice.id,
            description="Non-overlap charge",
            quantity=Decimal("1.000"),
            unit_price=amount,
            amount=amount,
            is_active=True,
...
def test_finds_overlapping_prepaid_invoice_and_reports_paid_coverage(
    ...
    paid = _paid_invoice(
        db_session, subscriber_account, subscription, paid_start, paid_end
    )
...
def test_partial_overlap_is_report_only_and_does_not_hold_or_void_invoice(
    db_session, subscriber_account, subscription
):
    paid_start = datetime(2026, 6, 4, tzinfo=UTC)
    paid_end = datetime(2026, 7, 3, tzinfo=UTC)
    ...
    _paid_invoice(db_session, subscriber_account, subscription, paid_start, paid_end)
    bad = _bad_invoice(db_session, subscriber_account, subscription, bad_start, bad_end)
    _misc_invoice_line(db_session, bad)
    db_session.commit()

    rows = find_prepaid_overlap_candidates(db_session)

    assert len(rows) == 1
    assert rows[0].action == "partial_overlap_manual_review"
    ...
    assert bad.status == InvoiceStatus.issued
    assert bad.balance_due == Decimal("100.00")
    assert not (bad.metadata_ or {}).get("reconciliation_hold")


def test_dunning_dry_run_does_not_write_overlap_hold_metadata(
    db_session, subscriber_account, subscription
):
    paid_start = datetime(2026, 6, 4, tzinfo=UTC)
    paid_end = datetime(2026, 7, 3, tzinfo=UTC)
    ...
    collections_service.dunning_workflow.run(
        db_session,
        DunningRunRequest(run_at=bad_start, dry_run=True),
    )
