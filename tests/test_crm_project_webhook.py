"""Inbound CRM project webhook endpoint: HMAC gate + delegation."""
...
def _post(db_session, body: dict, event: str = "project.created", signature=...):
    ...
    try:
        payload = _run(
            receive_crm_project_event(_FakeRequest(raw, headers), db_session)
        )
...
def test_valid_project_event_reaches_service(db_session):
    ...
    assert db_session.query(ProjectMirror).filter_by(crm_project_id="p-1").count() == 1


def test_bad_signature_rejected(db_session):
    body = {"subscriber_id": str(uuid.uuid4()), "project_id": "x"}
    with _with_secret(SECRET):
...
def test_unknown_event_ignored(db_session):
    body = {"subscriber_id": str(uuid.uuid4()), "project_id": "x"}
    with _with_secret(SECRET):
        code, resp = _post(db_session, body, event="project.archived")
    assert code == 200
