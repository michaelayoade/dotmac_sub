...
def test_resolution_refuses_when_the_party_owns_no_principal(db_session) -> None:
    ...
    for principal in (
        ...
        principal.person_party_id = None
    db_session.commit()
...
def test_resolution_refuses_an_ambiguous_party(db_session) -> None:
    """A Party owning two principals cannot say whose session this is.

    `uq_system_users_person_party_id` makes this unreachable on a migrated
    catalog, so reaching it means the constraint was lost — refuse, never pick.
    """

    _user, person, credential = _bound_login(db_session, "ambiguous@example.test")
    ...
    assert (
        exc.value.refusal
        is resolver.StaffProjectionRefusal.party_owns_multiple_principals
    )
