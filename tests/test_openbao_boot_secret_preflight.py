...
def test_optional_material_is_reported_and_never_gates() -> None:
    ...
    Failing a deploy over it would make the preflight useless for deployments
    that do not use that feature, so this reports and returns names while
    `main` keeps its exit code from the REQUIRED check alone.
    """
    ...
    assert report_optional_boot_material(refs, resolve) == ("anchor",)
...
def test_the_optional_set_is_the_one_the_application_holds_optionally() -> None:
    ...
    from scripts.setup.verify_openbao_boot_secrets import OPTIONAL_REFS, REQUIRED_REFS
