...
def test_advances_cannot_exceed_the_ceiling(db_session):
    """40% of a 1,000,000 quote is 400,000."""
    installation, vendor = _project(db_session)
    ...
    with pytest.raises(vendor_advances.VendorAdvanceError) as exc:
        vendor_advances.request_advance(
            db_session,
            vendor_advances.RequestVendorAdvance(
                project_id=installation.id, vendor_id=vendor.id, amount="400000.01"
            ),
        )
...
def test_advances_cannot_be_stacked_past_the_ceiling(db_session):
    """Each request must count what is already committed, or a vendor could
    take the full advance repeatedly."""
    installation, vendor = _project(db_session)
    vendor_advances.request_advance(
        db_session,
        vendor_advances.RequestVendorAdvance(
    ...
    with pytest.raises(vendor_advances.VendorAdvanceError) as exc:
        vendor_advances.request_advance(
            db_session,
            vendor_advances.RequestVendorAdvance(
                project_id=installation.id, vendor_id=vendor.id, amount="150000"
            ),
        )
