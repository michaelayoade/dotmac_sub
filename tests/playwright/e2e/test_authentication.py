...
def test_admin_authentication(api_context, admin_token: str):
    response = api_get(
        api_context, "/api/v1/auth/me", headers=bearer_headers(admin_token)
    )
    assert response.ok
...
def test_agent_authentication(api_context, agent_token: str):
    response = api_get(
        api_context, "/api/v1/auth/me", headers=bearer_headers(agent_token)
    )
    assert response.ok
    payload = response.json()
...
def test_user_authentication(api_context, user_token: str):
    response = api_get(
        api_context, "/api/v1/auth/me", headers=bearer_headers(user_token)
    )
    assert response.ok
    payload = response.json()
