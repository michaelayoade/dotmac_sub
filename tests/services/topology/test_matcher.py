...
def test_olt_matched_by_zabbix_host_id(db_session):
    olt = OLTDevice(name="OLT-1", hostname="olt1", mgmt_ip="10.0.0.1", zabbix_host_id="555")
    db_session.add(olt)
    db_session.flush()
...
def test_nas_matched_by_unique_management_ip(db_session):
    ...
    db_session.add(nas)
    db_session.flush()
    z = _zhost(hostid="900", name="NAS: edge", ips=("10.0.0.5",), groups=("DotMac/Network/NAS",))
    t, i, status = match_host(db_session, z)
    assert (t, i, status) == ("nas", nas.id, MATCHED)


def test_two_hosts_same_ip_disambiguated_by_group(db_session):
    # An OLT device host and its NAS host can share a management IP. Group /
    # name membership decides which provisioning device each maps to.
    ...
    olt_host = _zhost(hostid="201", name="olt2", ips=("10.0.0.8",), groups=("Garki BTS",))
    nas_host = _zhost(hostid="202", name="NAS: garki", ips=("10.0.0.8",), groups=("DotMac/Network/NAS",))
...
def test_no_candidate_is_unmatched(db_session):
    z = _zhost(hostid="777", name="ghost", ips=("192.168.99.99",), groups=("Garki BTS",))
...
def test_ambiguous_when_multiple_candidates(db_session):
    ...
    db_session.flush()
    z = _zhost(hostid="303", name="NAS: dup", ips=("10.9.9.9",), groups=("DotMac/Network/NAS",))
