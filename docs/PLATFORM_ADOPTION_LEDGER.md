...
| `dotmac_kernel.messaging` (+ `.envelope`, `.models`, `.outbox`, `.platform`, `.platform_relay`, `.platform_worker`, `.relay`, `.worker`) | storage-provider composed; runtime defer-db | S7+ | Migration 557 hosts and verifies the `outbox_events` / `platform_outbox_events` storage and relay-function contract for composed modules without installing a dispatcher. Inbox processing delegates to the idempotency owner below. Sub's `events.store`, owner-output and integration delivery paths retain every writer and worker until a separately sealed cutover retires them |
