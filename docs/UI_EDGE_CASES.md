...
- [x] `#31` Billing-mode drift is now resolved through the canonical billing profile and permanent enforcement owner. Mixed or unsafe profiles produce a typed no-action outcome; prepaid→postpaid transitions enter the repair cohort so obsolete prepaid timers and locks cannot strand access. Current behavior and verification live in `docs/FINANCIAL_ACCESS_ENFORCEMENT.md`.
...
- [!] `#7` empty-family instant change 400 — retired 2026-07-22; plan family is merchandising metadata and is no longer an eligibility or delivery gate
- [!] `#30` (found on PROD by driving account 100000016) Self-service plan change **ignored account debt** and the web/API paths disagreed. FIXED: `confirm_service_change` and `submit_change_plan` block prepaid and postpaid changes on the canonical collection-blocking balance; both web and `/me` use the same preview and confirmation owner. Commercial-only changes apply immediately. Remote and field delivery modes persist the confirmed intent without a plan swap, billing mutation, or ticket until provisioning/field verification. Insufficient prepaid funding remains a 402 in the API. The page shows the exact overdue amount and Pay-now CTA and hides the picker while in arrears; submit-time enforcement remains authoritative.
- [!] Same offer at the same address is excluded; same offer at a different address is an address-only field relocation
- [!] Cross-family change → delivery mode is derived from provisionable facts; remote and field verifier execution remains to be completed
- [!] Address-only relocation keeps the current plan but is always field delivery. Fixed-wireless/radio relocation is blocked unless the exact target address qualifies and a configured catalog-backed nonzero one-time charge is quoted. Confirmation stores immutable quote evidence as `awaiting_payment`; it does not move the subscription before payment and verified field delivery.
