...
| `financial.service_extensions` | service-extension lifecycle and exact grant intervals | `command_writer` | authenticated extension command ← `auth.permission_gate`<br>canonical subscription lifecycle and billing anchor ← `access.subscription_lifecycle`<br>service-extension duration policy ← `control.settings_spec` | `owner_managed` | `complete` | billing and customer operations | `docs/designs/SERVICE_EXTENSION_EFFECTIVE_INTERVALS.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_service_extensions.py`<br>`tests/test_prepaid_service_coverage.py`<br>`tests/architecture/test_service_extension_boundary.py` |
| `financial.service_extensions` | service-extension billing-anchor projection | `projection_writer` | exact service-extension grant interval ← `financial.service_extensions`<br>canonical subscription lifecycle and billing anchor ← `access.subscription_lifecycle` | `owner_managed` | `complete` | billing and customer operations | `docs/designs/SERVICE_EXTENSION_EFFECTIVE_INTERVALS.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_service_extensions.py`<br>`tests/test_prepaid_service_coverage.py`<br>`tests/architecture/test_service_extension_boundary.py` |
