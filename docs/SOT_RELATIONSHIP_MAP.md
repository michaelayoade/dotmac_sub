...
| `operations.service_team_party_cutover` | service-team Party cutover readiness | `resolver` | current native service-team cutover state ← `operations.service_team_lifecycle`<br>canonical Person Party identity ← `party.registry`<br>staff authentication principal state ← `auth.staff_provisioning`<br>legacy workflow team settings ← `control.settings_spec` | `coordinator_managed` | `cutover_ready` | operations administration and identity governance | `docs/designs/SERVICE_TEAM_LIFECYCLE_SOT.md`<br>`docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/runbooks/SERVICE_TEAM_PARTY_CUTOVER.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_service_team_party_cutover.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/architecture/test_service_team_party_cutover_boundary.py` |
| `operations.service_team_party_cutover` | approved service-team Party cutover adoption | `application_coordinator` | CRM service-team identity snapshot ← `external:dotmac_crm`<br>reviewed service-team identity decisions ← `operations.service_team_party_cutover`<br>current native service-team cutover state ← `operations.service_team_lifecycle`<br>canonical Person Party identity ← `party.registry`<br>staff authentication principal state ← `auth.staff_provisioning` | `coordinator_managed` | `cutover_ready` | operations administration and identity governance | `docs/designs/SERVICE_TEAM_LIFECYCLE_SOT.md`<br>`docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/runbooks/SERVICE_TEAM_PARTY_CUTOVER.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_service_team_party_cutover.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/architecture/test_service_team_party_cutover_boundary.py` |
