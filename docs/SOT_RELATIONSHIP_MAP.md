...
persistence-like mutation must name a declared owner. The 218 existing
...
| `operations.field_notes` | native field work-order note creation | `command_writer` | authenticated technician identity ← `auth.permission_gate`<br>assigned work-order state ← `operations.work_orders`<br>field-note client request identity ← `operations.field_notes` | `owner_managed` | `cutover_ready` | field operations | `docs/designs/FIELD_WORK_ORDER_NOTES.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_field_notes.py`<br>`tests/architecture/test_field_note_delivery_contract.py`<br>`tests/integration/test_field_note_delivery_migration.py`<br>`field_mobile/test/execution_test.dart`<br>`field_mobile/test/jobs_screens_test.dart` |
| `operations.field_notes` | authorized staff field-note related-context projection | `resolver` | canonical field work-order note state ← `operations.field_notes`<br>native work-order relationship bindings ← `operations.work_order_commands`<br>staff field-note authorization evidence ← `auth.permission_gate` | `read_only` | `cutover_ready` | field operations and support | `docs/designs/FIELD_WORK_ORDER_NOTES.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_field_note_staff_projection.py`<br>`tests/architecture/test_field_note_delivery_contract.py` |
