...
| `communications.team_inbox_customer_completion_policy` | immutable Customer resolution-completion policy versions and the active identity-guard decision | `command_writer` | authorized Inbox settings decision ← `auth.permission_gate` | `owner_managed` | `complete` | customer experience platform | `docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_inbox_customer_completion.py` |
