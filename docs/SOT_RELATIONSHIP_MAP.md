...
persistence-like mutation must name a declared owner. The 215 existing
...
| `customer.search` | bounded active customer search and customer identity selection | `resolver` | typed customer search query ← `customer.search`<br>selected canonical customer identities ← `customer.accounts` | `read_only` | `native` | customer operations | `docs/designs/AUTOMATION_CENTER_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_customer_search_services.py`<br>`tests/architecture/test_customer_search_performance.py` |
...
| `financial.customer_chargeability` | typed customer chargeability classification | `resolver` | canonical current subscription scope ← `access.subscription_lifecycle`<br>canonical recurring catalog price ← `service_intent.catalog_policy`<br>effective non-standard billing treatment ← `financial.subscription_billing_treatments`<br>chargeability classification protocol ← `financial.customer_chargeability` | `read_only` | `complete` | finance and customer operations | `docs/designs/CUSTOMER_CHARGEABILITY_AND_BILLING_MODE_TRANSITIONS.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FRONTEND_SPEC.md`<br>`tests/test_customer_chargeability.py`<br>`tests/test_web_customer_lists.py`<br>`tests/architecture/test_billing_profile_boundary.py` |
| `financial.customer_chargeability` | non-billable and pricing-review cohort predicate | `resolver` | canonical current subscription scope ← `access.subscription_lifecycle`<br>canonical recurring catalog price ← `service_intent.catalog_policy`<br>effective non-standard billing treatment ← `financial.subscription_billing_treatments`<br>chargeability classification protocol ← `financial.customer_chargeability` | `read_only` | `complete` | finance and customer operations | `docs/designs/CUSTOMER_CHARGEABILITY_AND_BILLING_MODE_TRANSITIONS.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FRONTEND_SPEC.md`<br>`tests/test_customer_chargeability.py`<br>`tests/test_web_customer_lists.py`<br>`tests/architecture/test_billing_profile_boundary.py` |
...
| `financial.billing_profile` | billing-mode transition policy | `policy` | canonical account billing mode ← `customer.accounts`<br>canonical collectible subscription billing modes ← `access.subscription_lifecycle`<br>canonical offer billing-mode availability ← `service_intent.catalog_policy`<br>billing profile protocol ← `financial.billing_profile` | `read_only` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/audits/BILLING_SOT_AUDIT_2026-07-12.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_billing_profile.py`<br>`tests/test_shared_policy_services.py`<br>`tests/test_billing_cleanup_remediation.py`<br>`tests/architecture/test_billing_profile_boundary.py` |
| `financial.billing_mode_transition` | billing-mode transition eligibility and impact preview | `resolver` | billing-mode transition command evidence ← `financial.billing_mode_transition`<br>canonical billing profile ← `financial.billing_profile`<br>canonical customer chargeability ← `financial.customer_chargeability`<br>canonical account and subscription lifecycle ← `access.subscription_lifecycle`<br>canonical target-mode offer availability ← `service_intent.catalog_policy`<br>canonical customer financial position ← `customer.financial_position`<br>canonical invoice state ← `financial.invoices`<br>effective subscription billing treatment ← `financial.subscription_billing_treatments`<br>pending subscription plan change ← `service_intent.subscription_change_execution` | `coordinator_managed` | `complete` | billing operations | `docs/designs/CUSTOMER_CHARGEABILITY_AND_BILLING_MODE_TRANSITIONS.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_billing_mode_transitions.py`<br>`tests/architecture/test_billing_profile_boundary.py` |
| `financial.billing_mode_transition` | account-wide billing-mode transition | `application_coordinator` | billing-mode transition command evidence ← `financial.billing_mode_transition`<br>signed billing-mode transition preview ← `financial.billing_mode_transition`<br>canonical billing profile ← `financial.billing_profile`<br>canonical customer chargeability ← `financial.customer_chargeability`<br>canonical account and subscription lifecycle ← `access.subscription_lifecycle`<br>canonical target-mode offer availability ← `service_intent.catalog_policy`<br>canonical customer financial position ← `customer.financial_position`<br>canonical invoice state ← `financial.invoices`<br>effective subscription billing treatment ← `financial.subscription_billing_treatments`<br>pending subscription plan change ← `service_intent.subscription_change_execution` | `coordinator_managed` | `complete` | billing operations | `docs/designs/CUSTOMER_CHARGEABILITY_AND_BILLING_MODE_TRANSITIONS.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_billing_mode_transitions.py`<br>`tests/architecture/test_billing_profile_boundary.py` |
...
| `financial.prepaid_service_renewals` | prepaid settlement service-period resolution | `resolver` | settled payment evidence ← `financial.payments`<br>prepaid subscription and renewal terms ← `billing.contracts`<br>usage allowance reset policy ← `service_intent.catalog_policy`<br>funded service entitlement evidence ← `financial.prepaid_service_renewals`<br>applied service-extension coverage evidence ← `financial.service_extensions` | `owner_managed` | `cut_over` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/FUNDED_PREPAID_RENEWAL_INVOICING.md`<br>`docs/designs/USAGE_ALLOWANCE_RESET_CYCLES.md`<br>`docs/runbooks/LEGACY_PREPAID_RENEWAL_TAX_INVOICE_CORRECTION.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/services/billing/test_payment_status_recompute.py`<br>`tests/test_subledger_forward_shadow.py`<br>`tests/architecture/test_prepaid_billing_anchor_ownership.py` |
...
| `communications.zeptomail_delivery_reconciliation` | ZeptoMail provider delivery reconciliation | `reconciler` | authenticated ZeptoMail delivery observation ← `external:zeptomail`<br>submitted notification client reference ← `communications.notification_service` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_zeptomail_delivery_tracking.py` |
...
| `automation.rule_definitions` | automation rule definitions and immutable versions | `authoritative_record` | typed automation rule lifecycle command ← `automation.rule_definitions`<br>declared automation capabilities ← `automation.capability_registry`<br>tenant-scoped automation rule records ← `automation.rule_definitions`<br>company-wide or selected-customer rule scope ← `UNKNOWN INPUT`<br>active ticket assignment rules ← `support.ticket_assignment_rule_configuration`<br>active ticket-creation automation rules ← `support.ticket_automation_rule_configuration`<br>active Automation Center rules for the same trigger and action ← `automation.rule_definitions` | `owner_managed` | `native` | platform automation | `docs/designs/AUTOMATION_CENTER_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_automation_rules.py`<br>`tests/architecture/test_automation_rule_boundary.py` |
...
| `operations.expense_requests` | dead submitted expense delivery retry | `command_writer` | canonical submitted expense request ← `operations.expense_requests`<br>validated receipt content ← `operations.expense_requests`<br>failed expense submission delivery evidence ← `integration.backoffice_adapter`<br>expense ERP delivery cutover control ← `integration.backoffice_adapter` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
...
| `usage.quota_cycle_policy` | catalogue-driven quota cycle and rollover policy resolution | `resolver` | usage allowance reset policy ← `service_intent.catalog_policy`<br>funded subscription interval ← `financial.prepaid_service_renewals`<br>RADIUS accounting facts ← `sessions.radius_reconciliation` | `read_only` | `complete` | billing and network access | `docs/designs/USAGE_ALLOWANCE_RESET_CYCLES.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_usage_metering.py`<br>`tests/test_usage_rollover.py`<br>`tests/test_prepaid_service_renewals.py` |
...
- Prepaid settlement calendar boundary: a calendar-month prepaid settlement
  ...
  `renewal_cycle` allowance instead restarts at the exact payment instant
  for its configured `validity_days`, even when prior coverage remains.
  `financial.prepaid_service_renewals` resolves the service interval and
  `usage.quota_cycle_policy` resolves the matching quota bucket and crossing-
  session counter baselines in the same transaction.
...
9. `operations.vendor_project_lifecycle` (`app.services.vendor_portal_operations`)
   ...
   `draft -> assigned` (direct assignment), and pre-field-work
   `assigned`/`approved -> draft` vendor unassignment or award cancellation,
   vendor
   ...
   `vendor_project.unassigned`, `vendor_quote.changed` with `award_canceled`
   when an approved quote is canceled, `vendor_project.verified`, or
   `vendor_project.rework_requested`. Cross-team consumers
...
requested move. Technology changes also carry the customer's selected,
compatible destination offer; same-technology moves retain the current offer.
Customer reads show coverage and review status while hiding
price and deposit fields until the current Quote snapshot has staff approval.
`sales.quote_payment_review` rejects approval before a priced line, positive
total, and deposit policy are present. Quote intake never changes the existing
Subscription. Relocation quotes require a 100% charge and cannot use the
installation deposit payment path. The approved Quote total is authoritative
for this customer service-request relocation; the configured Airfiber fee
continues to govern the separate plan-change preview path. The typed handoff
to `service_intent.subscription_change_execution` creates one exact Invoice
and binds the destination Address, qualification, Subscription and offer.
That owner alone releases fulfillment after settlement of the full Invoice.
Verified field completion applies the selected address and offer without
creating a second plan-proration charge.
