...
| `financial.billing_tax_reconciliation` | legacy VAT reconciliation candidate policy | `policy` | canonical issued invoice tax evidence ← `financial.invoices`<br>canonical issued credit-note tax adjustments ← `financial.credit_notes`<br>canonical customer VAT exemption policy ← `financial.customer_tax_policies`<br>canonical subscription invoice-line scope ← `access.subscription_lifecycle`<br>catalog compatibility VAT fields ← `service_intent.catalog_policy` | `read_only` | `complete` | billing and finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/CUSTOMER_VAT_EXEMPTION.md`<br>`docs/PLAN_FAMILY_ARCHITECTURE.md`<br>`tests/test_billing_tax_reconciliation.py`<br>`tests/architecture/test_billing_tax_reconciliation_boundary.py` |
| `financial.billing_tax_reconciliation` | bounded legacy VAT reconciliation projection | `resolver` | canonical issued invoice tax evidence ← `financial.invoices`<br>canonical issued credit-note tax adjustments ← `financial.credit_notes`<br>canonical customer VAT exemption policy ← `financial.customer_tax_policies`<br>canonical subscription invoice-line scope ← `access.subscription_lifecycle`<br>catalog compatibility VAT fields ← `service_intent.catalog_policy` | `read_only` | `complete` | billing and finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/CUSTOMER_VAT_EXEMPTION.md`<br>`docs/PLAN_FAMILY_ARCHITECTURE.md`<br>`tests/test_billing_tax_reconciliation.py`<br>`tests/architecture/test_billing_tax_reconciliation_boundary.py` |
...
| `service_intent.subscription_change_execution` | pending service-change cancellation | `command_writer` | canonical subscription-change execution state ← `service_intent.subscription_change_execution` | `coordinator_managed` | `complete` | customer service delivery, billing, and network operations | `docs/designs/CUSTOMER_SELF_SERVICE_LIFECYCLE.md`<br>`docs/designs/PROVISIONING_LIFECYCLE_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_customer_plan_change_prepaid.py`<br>`tests/architecture/test_provisioning_lifecycle_sot.py` |
