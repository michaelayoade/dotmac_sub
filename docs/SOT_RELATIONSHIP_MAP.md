...
| `network.cabinet_notice` | operator-initiated cabinet service notices | `command_writer` | tokenized cabinet audience ← `network.outage_impact`<br>customer notification policy decisions ← `communications.customer_policy`<br>operator notice command ← `network.cabinet_notice` | `participant` | `complete` | network operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_cabinet_notice.py` |
