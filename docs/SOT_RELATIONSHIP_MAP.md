...
| `integration.dotmac_erp_operational_context_adapter` | typed ERP operational-context projection and per-domain delivery watermarks | `projection_writer` | canonical projects and project tasks ← `operations.project_lifecycle`<br>canonical tickets ← `support.ticket_lifecycle`<br>canonical work orders ← `operations.work_order_commands`<br>configured ERP capability ← `integration.backoffice_adapter`<br>ERP version-2 response ← `external:dotmac_erp` | `owner_managed` | `cutover_ready` | service delivery integrations | `docs/designs/CONFIGURABLE_ERP_OPERATIONAL_SYNC.md`<br>`tests/test_dotmac_erp_domain_sync.py` |
