...
2. `integration.installations`: solely owns version-pinned installation
   ...
   bindings for platform-managed connectors. Manifest changes require explicit
   prior-pin adoption; post-migration deployment verification rejects any
   enabled installation whose version or digest does not match the candidate
   registry.
