...
are intentionally outside Self-Care. The supported NCC complaints CSV is
unrelated and remains native.

| Capability | Self-Care owner/query | Route | Permission | UI/test state |
| --- | --- | --- | --- | --- |
| NCC complaints and Tuesday weekly CSV delivery | `compliance.ncc_complaints_reporting`; `communications.ncc_weekly_delivery` | `/admin/reports/ncc-complaints`; exact scheduled artifact under `/ncc-weekly-runs/{run_id}/download` | `reports:ncc:read`; export/artifact requires `reports:ncc:export`; configuration requires `notification:write` | Native weekly page/export plus configurable Tuesday schedule, To/CC/BCC/body/sender parity, durable artifact and run evidence; disabled until controlled cutover |
