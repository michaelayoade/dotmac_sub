...
| 2 | **Connector secrets plaintext at rest** | `app/models/connector.py:58` `auth_config` plain JSON; write path applies no `encrypt_credential` | **FIXED** — #534 `EncryptedJSON` type encrypts on write / tolerant read; pre-existing plaintext prod rows re-encrypted + round-trip verified 2026-07-03 |
