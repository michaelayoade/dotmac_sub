...
Mobile Help chat photo messages use the same visitor message owner and the same
`InboxMessage` chronology as text. The typed visitor command accepts at most five
JPEG, PNG, GIF, or WebP images of at most 5 MB each. The owner validates them,
stages private stored files, binds their Inbox media assets to the message, and
records the message and attachment references in one transaction. A photo-only
message is valid; AI intake runs only when the message has text. A client message
key is checked under the conversation lock against a body and photo fingerprint;
an exact retry returns the original result, while changed content conflicts.
History returns bounded attachment identity and filename metadata. The visitor media read path
requires the exact signed session and conversation, a bound non-internal image,
and serves private bytes without caching. The mobile composer offers a plus
button with Camera and Gallery choices, removable previews, and retry of failed
uploads.
