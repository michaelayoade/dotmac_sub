...
| AI intake policy | `ai.intake` | Stores classification/workflow/context policy; off by default and never writes Inbox state |
| Manager AI insight | `communications.team_inbox_manager_ai_chat` | Builds read-only manager chat context and returns advisory answers without Inbox mutations |
...
AI intake configuration is a default-off control plane. It may name prompt
instructions, ordered workflow steps, context sources, department mappings,
handoff policy, and assignment strategy. The inbound observation processor calls
the automation only after a new customer message is accepted, and only inside an
optional owner savepoint. When the intake gate or global AI gates are off, no
LLM call is made. When enabled, every customer-visible consequence remains under
Team Inbox owners: replies use outbound intent, handoff uses routing assignment,
and close/end-conversation status changes remain Team Inbox state transitions.

Manager AI insight is separate from customer-facing intake. The
`support:inbox_ai:read` page reads a selected conversation or recent queue
snapshot, calls the configured AI provider for analysis, and returns the answer
without creating messages, assignments, tickets, labels, or status transitions.
