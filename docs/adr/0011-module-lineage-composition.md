...
set is derived from the composed lineages rather than listed —
`docs/generated/MODULE_SCHEMA_CONTRACT.md`, regenerated by
`make schema-contract` (see the 2026-08-31 amendment); the restricted
migration role must not retain database-level `CREATE` merely so an
already-present module schema can be observed.
...
## Amendment — 2026-08-31: the module schema set is derived, and repair can fail

The schema set was maintained as prose in three places — this ADR, the
production runbook and the adoption ledger. All three had drifted and none of
them named `mod_inbox`, which is how it reached production unprovisioned and
cost two release candidates. Prose is not a manifest. The set is now derived
from the composed lineages in `alembic.ini` through each distribution's
declared `short_code` and `dotmac_kernel.namespaces.module_schema`, rendered
into `docs/generated/MODULE_SCHEMA_CONTRACT.md` by
`make schema-contract` and failed on drift by `make schema-contract-check`.
Adding a module to `alembic.ini` therefore adds it to the bootstrap, the
verifier and the document in the same act. No document may enumerate the
schemas again.

The prerequisite decision itself is unchanged: schemas remain a privileged
deployment prerequisite and the restricted migration role still holds no
database-level `CREATE`. What changes is that its repair leg can now fail
honestly. `scripts/deploy.sh` probes the contract with the restricted
connection and reports `already_satisfied`, `repaired`, or `blocked`; a
`blocked` deploy refuses and stops before migrations, where it previously
returned success whenever no elevated credential was configured. Deployment
repair holds a dedicated NOCREATEROLE cluster role that can create schemas
owned by `dotmac_app` and nothing else, so `--repair-schemas` is the
deployment's mode and the elevated `--repair` remains one-off operator
provisioning. A missing cluster role is reported as `blocked`, never worked
around.
