...
def speedtests_list(
    ...
    data, load_error = web_network_speedtests_service.list_page_data_safe(
        db,
        search=search,
...
def speedtests_create(request: Request, db: Session = Depends(get_db)):
    ...
    result = web_network_speedtests_service.create_speedtest_from_values(
...
def speedtests_clear_history(request: Request, db: Session = Depends(get_db)):
    ...
    if error:
        return RedirectResponse(
            f"/admin/network/speedtests?error_message={quote_plus(error)}",
            status_code=303,
        )
