...
def fiber_strands_list(request: Request, db: Session = Depends(get_db)):
    ...
    context = _base_context(
        request, db, active_page="fiber-strands", active_menu="fiber"
    )
...
def fiber_strand_create(request: Request, db: Session = Depends(get_db)):
    ...
    if error:
        ...
        return templates.TemplateResponse(
            "admin/network/fiber/strand-form.html", context
        )
...
def fiber_strand_update(
    ...
    try:
        ...
        updated_strand = web_network_strands_service.update_strand(
            db, strand_id, values
        )
...
def splice_closures_list(request: Request, db: Session = Depends(get_db)):
    ...
    context = _base_context(
        request, db, active_page="splice-closures", active_menu="fiber"
    )
    context.update(page_data)
    return templates.TemplateResponse(
        "admin/network/fiber/splice-closures.html", context
    )
...
def splice_closure_new(request: Request, db: Session = Depends(get_db)):
    ...
    context.update(form_context)
    return templates.TemplateResponse(
        "admin/network/fiber/splice-closure-form.html", context
    )
...
def splice_closure_create(request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        f"/admin/network/splice-closures/{closure.id}", status_code=303
    )
...
def splice_closure_detail(
    ...
    context["activities"] = build_audit_activities(
        db, "splice_closure", str(closure_id), limit=10
    )
    return templates.TemplateResponse(
        "admin/network/fiber/splice-closure-detail.html", context
    )
...
def splice_tray_new(request: Request, closure_id: str, db: Session = Depends(get_db)):
    ...
    context.update(form_context)
    return templates.TemplateResponse(
        "admin/network/fiber/splice-tray-form.html", context
    )
...
def splice_tray_redirect(closure_id: str):
    return RedirectResponse(
        f"/admin/network/splice-closures/{closure_id}", status_code=303
    )
...
def splice_tray_create(
    ...
    try:
        tray = web_network_splice_closures_service.create_tray(
            db, str(closure.id), values
        )
        from app.web.admin import get_current_user
...
def splice_tray_update(
    ...
    _, error = web_network_splice_closures_service.validate_tray_form_values(values)
    if error:
        tray_data = web_network_splice_closures_service.tray_form_data(
            values, tray_id=str(tray.id)
        )
        form_context = web_network_splice_closures_service.build_tray_form_context(
            closure=closure,
            tray=tray_data,
...
def splice_create(request: Request, closure_id: str, db: Session = Depends(get_db)):
    ...
    if error:
        ...
        context.update(form_context)
        return templates.TemplateResponse(
            "admin/network/fiber/splice-form.html", context
        )

    try:
        splice = cast(
            FiberSplice,
            web_network_splice_closures_service.create_splice(
                db, str(closure.id), values
            ),
        )
        from app.web.admin import get_current_user
        ...
        log_audit_event(
            ...
            metadata={
                ...
                if splice.from_strand_id
                else None,
                "to_strand_id": str(splice.to_strand_id)
                if splice.to_strand_id
                else None,
            },
        )
        return RedirectResponse(
