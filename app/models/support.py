...
class Ticket(Base):
    ...
    customer_account_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    lead_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True))
    customer_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    created_by_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    assigned_to_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    technician_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    ticket_manager_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    site_coordinator_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    ...
    tags: Mapped[list[str] | None] = mapped_column(MutableList.as_mutable(JSON()), default=list)
    metadata_: Mapped[dict | None] = mapped_column("metadata", MutableDict.as_mutable(JSON()))
    attachments: Mapped[list[dict] | None] = mapped_column(MutableList.as_mutable(JSON()), default=list)
    ...
    sla_events = relationship("TicketSlaEvent", back_populates="ticket", cascade="all, delete-orphan")
...
class TicketComment(Base):
    ...
    author_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
...
class TicketSlaEvent(Base):
    ...
    __table_args__ = (
        Index("ix_support_ticket_sla_events_ticket", "ticket_id"),
    )
...
class TicketMerge(Base):
    ...
    __table_args__ = (
        UniqueConstraint("source_ticket_id", "target_ticket_id", name="uq_support_ticket_merge_pair"),
    )
    ...
    merged_by_person_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("subscribers.id"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
...
class TicketLink(Base):
    ...
    __table_args__ = (
        UniqueConstraint("from_ticket_id", "to_ticket_id", "link_type", name="uq_support_ticket_link"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    from_ticket_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("support_tickets.id"), nullable=False)
    to_ticket_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("support_tickets.id"), nullable=False)
