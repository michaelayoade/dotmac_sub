...
def run_enforcement_reconciler() -> dict[str, int]:
    ...
    with psycopg.connect(dsn) as rconn, rconn.cursor() as cur:
        cur.execute(
            ...
            "host(r.framedipaddress) FROM radacct r "
    ...
    try:
        ...
        for username, session_id, nas_ip, framed_ip in to_kick.values():
        ...
        expected_wg = {
            ...
            or sub_status
            in (SubscriptionStatus.blocked, SubscriptionStatus.suspended)
        }
    finally:
        db.close()
