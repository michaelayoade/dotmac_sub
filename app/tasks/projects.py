"""Celery tasks for the local project/installation mirror."""
...
@celery_app.task(name="app.tasks.projects.reconcile_project_mirror")
def reconcile_project_mirror() -> dict[str, int]:
    """Reconcile stale local project mirrors against the CRM (backstop for missed
    webhook deliveries). Returns {reconciled: N}."""
    logger.info("Starting reconcile_project_mirror")
    db = db_session_adapter.create_session()
    try:
        from app.services import projects_mirror

        count = projects_mirror.reconcile_all(db)
        logger.info("Completed reconcile_project_mirror: reconciled=%s", count)
