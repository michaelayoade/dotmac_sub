...
class CanonicalCustomerProfileField(StrEnum):
    """Customer fields writable by a typed, transaction-neutral participant."""
...
class CanonicalCustomerProfilePatchError(DomainError):
    """The canonical Customer account owner refused a profile patch."""
...
def apply_canonical_customer_profile_patch(
    ...
    """Apply validated Customer fields under the ``customer.accounts`` owner.

    This is a flush-only participant for registered cross-domain coordinators.
    It deliberately cannot create a Customer and never completes the caller's
    transaction.
    """
