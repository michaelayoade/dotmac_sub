...
def get_tr069_profiles_for_olt(
    ...
    *,
    ...
    """Return OLT TR-069 profiles using Redis TTL, then live SSH, then DB fallback."""
    ...
    snapshot = olt.tr069_profiles_snapshot if isinstance(
        olt.tr069_profiles_snapshot, dict
    ) else {}
    profiles = _profiles_from_payload(snapshot.get("profiles"))
    fetched_at = _parse_datetime(snapshot.get("fetched_at")) or olt.tr069_profiles_snapshot_at
    if profiles:
        return ObservedReadResult(
            ok=True,
...
def get_cached_iphost_config(ont: OntUnit) -> ObservedReadResult | None:
    snapshot = ont.olt_observed_snapshot if isinstance(ont.olt_observed_snapshot, dict) else {}
    config = snapshot.get("iphost_config")
    if not isinstance(config, dict) or not config:
        return None
    fetched_at = _parse_datetime(snapshot.get("iphost_fetched_at")) or ont.olt_observed_snapshot_at
    return ObservedReadResult(
        ok=True,
