...
def billing_path_coverage(db: Session) -> tuple[int, int]:
    ...
    """
    billable_statuses = tuple(BILLABLE_SUBSCRIBER_STATUS_VALUES)
    statuses_param = bindparam("billable_statuses", expanding=True)
    terminal = (
        db.execute(
            text(
                """
    ...
    no_path = (
        db.execute(
            text(no_path_sql).bindparams(statuses_param),
            {"billable_statuses": billable_statuses},
        ).scalar()
        or 0
    )
