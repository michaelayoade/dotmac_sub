...
class TopologyGaps:
    ...
    def unmatched_node_total_pages(self) -> int:
        return _total_pages(self.unmatched_node_total_count, self.unmatched_node_per_page)
...
def _subscription_gap_rows(db: Session) -> tuple[int, list[dict]]:
    ...
    for row in active_subs:
        ...
        if selected_ont_id is None and row.provisioning_nas_device_id in existing_nas_ids:
