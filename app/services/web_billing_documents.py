"""Billing document inventory page data — a projection over the billing SOT owners.
...
def _money(currency: object, amount: object) -> str:
    try:
        return f"{currency or 'NGN'} {Decimal(str(amount or 0)):,.2f}"
...
def billing_documents_data(db: Session, facet: str = "invoices") -> dict:
    """Return the billing-document page data for one billing facet (from its owner)."""
