...
def export_content(
    ...
    *,
    ...
    if normalized == "xlsx":
        xlsx_body = _render_xlsx(fields, data_rows, include_headers=include_headers)
        return (
            xlsx_body,
    ...
    if normalized == "pdf":
        ...
        pdf_body = _render_pdf(
            module_label, fields, data_rows, include_headers=include_headers
        )
        return pdf_body, "application/pdf", "pdf", len(data_rows)
...
def _send_export_email(
    ...
    *,
    ...
    try:
        ...
        sender_email = str(config.get("from_email") or "").strip()
        if not sender_email:
            return False
