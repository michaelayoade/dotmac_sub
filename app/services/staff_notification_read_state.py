...
def _repair_legacy_assignment_target(
    ...
    if notification.title.startswith("Ticket assigned: "):
        ...
        try:
            ...
            pass
        matches = db.query(Ticket).filter(or_(*filters)).limit(2).all()
        if len(matches) == 1:
            notification.target_url = f"/admin/support/tickets/{matches[0].id}"
        return notification.target_url
    ...
    if notification.title.startswith("Project assigned: "):
        ...
        try:
            ...
            pass
        matches = db.query(Project).filter(or_(*filters)).limit(2).all()
        if len(matches) == 1:
            notification.target_url = f"/admin/projects/{matches[0].id}"
        return notification.target_url
