...
void main() {
  ...
  group('vendor repository', () {
    ...
    test(
      'submit queues an as_built outbox entry with the right shape',
      () async {
        final recorder = TraceRecorder()..start();
        recorder.addPoint((latitude: 6.0, longitude: 3.0));
        recorder.addPoint((latitude: 6.001, longitude: 3.001));
        ...
        final payload = (jsonDecode(rows.single.payloadJson) as Map)
            .cast<String, dynamic>();
    ...
    test(
      'submit includes variation type and line items when supplied',
      () async {
        final recorder = TraceRecorder()..start();
        recorder.addPoint((latitude: 6.0, longitude: 3.0));
        recorder.addPoint((latitude: 6.001, longitude: 3.001));
    ...
    test('openQuoteDraft posts to the project quote endpoint', () async {
      adapter.on(
        'POST',
        '/api/v1/field/projects/p-1/quote',
        (_) => (
          200,
          {'id': 'q-1', 'status': 'draft', 'total': 0, 'currency': 'NGN'},
        ),
      );
    ...
    test(
      'fetchQuote parses the quote, line items and proposed routes',
      () async {
        adapter.on(
          'GET',
          '/api/v1/field/quotes/q-1',
          (_) => (
            200,
            {
              'quote': {
    ...
    test('submitQuote posts to the submit endpoint', () async {
      adapter.on(
        'POST',
        '/api/v1/field/quotes/q-1/submit',
        (_) => (
          200,
          {
            'id': 'q-1',
