...
class VendorProjectListItem {
  ...
  factory VendorProjectListItem.fromJson(Map<String, dynamic> json) =>
      VendorProjectListItem(
        project: VendorProject.fromJson(
          (json['project'] as Map).cast<String, dynamic>(),
        ),
        lifecycle: json['lifecycle'] != null
            ? VendorLifecycle.fromJson(
...
class AsBuiltSubmission {
  const AsBuiltSubmission({
    required this.id,
    required this.status,
    this.actualLengthMeters,
    this.reviewNotes,
  });
  ...
  factory AsBuiltSubmission.fromJson(Map<String, dynamic> json) =>
      AsBuiltSubmission(
        id: json['id'] as String,
...
class VendorQuoteDetail {
  ...
  factory VendorQuoteDetail.fromJson(Map<String, dynamic> json) =>
      VendorQuoteDetail(
        quote: VendorQuote.fromJson(
...
class VendorProjectDetail {
  ...
  factory VendorProjectDetail.fromJson(Map<String, dynamic> json) =>
      VendorProjectDetail(
        project: VendorProject.fromJson(
          (json['project'] as Map).cast<String, dynamic>(),
        ),
        ...
        rejectedForResubmission: json['rejected_for_resubmission'] != null
            ? AsBuiltSubmission.fromJson(
                (json['rejected_for_resubmission'] as Map)
                    .cast<String, dynamic>(),
              )
            : null,
      );
...
class VendorRepository {
  ...
  Future<List<VendorProjectListItem>> fetchProjects() async {
    ...
    return items
        .map(
          (item) =>
              VendorProjectListItem.fromJson(item.cast<String, dynamic>()),
        )
        .toList();
  }
  ...
  Future<VendorProjectDetail> fetchDetail(String projectId) async {
    final response = await _ref
        .read(apiClientProvider)
        .dio
        .get('/api/v1/field/projects/$projectId');
    return VendorProjectDetail.fromJson(
    ...
    await _ref
        ...
        .enqueue(
          ...
          payload: {
            ...
            'geojson': geojson,
            'actual_length_meters': double.parse(
              actualLengthMeters.toStringAsFixed(1),
            ),
  ...
  Future<VendorQuoteDetail> fetchQuote(String quoteId) async {
    ...
    return VendorQuoteDetail.fromJson(
      (response.data as Map).cast<String, dynamic>(),
    );
  }
  ...
  Future<void> addProposedRoute(
    ...
    await _dio.post(
      ...
      data: {
        'geojson': geojson,
        'length_meters': double.parse(lengthMeters.toStringAsFixed(1)),
      },
    );
  }
...
final vendorRepositoryProvider = Provider<VendorRepository>(
  VendorRepository.new,
);

final vendorProjectsProvider = FutureProvider<List<VendorProjectListItem>>(
  (ref) => ref.watch(vendorRepositoryProvider).fetchProjects(),
);

final vendorProjectDetailProvider =
    FutureProvider.family<VendorProjectDetail, String>(
      (ref, projectId) =>
          ref.watch(vendorRepositoryProvider).fetchDetail(projectId),
    );
...
final vendorProjectQuoteProvider =
    FutureProvider.family<VendorQuoteDetail, String>((ref, projectId) async {
