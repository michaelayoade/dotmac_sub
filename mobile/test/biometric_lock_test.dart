...
void main() {
  ...
  group('TokenStorage biometric flag', () {
    test(
      'survives a token clear (session-expiry) but disable removes it',
      () async {
  ...
  group('AuthController biometric lock', () {
    ...
    test(
      'token + opted-in + biometrics available -> locked on launch',
      () async {
        final ts = TokenStorage();
        ...
        final c = build(
          ts,
          _FakeBiometric(available: true),
          _FakeAuthRepository(ts),
        );
        await c.read(authControllerProvider.notifier).bootstrap();
    ...
    test(
      'opted-in but biometrics unavailable -> not locked (no trap)',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
        final c = build(
          ts,
          _FakeBiometric(available: false),
          _FakeAuthRepository(ts),
        );
        await c.read(authControllerProvider.notifier).bootstrap();
    ...
    test(
      'logout clears the biometric opt-in and any stashed location',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
    ...
    test(
      'bootstrap resolving /auth/me after an unlock must not re-lock',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
        ...
        expect(
          c.read(authControllerProvider).locked,
          isTrue,
    ...
    test(
      'bootstrap does not resurrect a session signed out mid-flight',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
        ...
        await boot;
        expect(
          c.read(authControllerProvider).isAuthenticated,
          isFalse,
    ...
    test(
      'promptActive is true only while the unlock prompt is in flight',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
    ...
    test(
      'lockOnResume locks synchronously when armed (no content flash)',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
    ...
    test(
      'lockOnResume rolls back when biometrics became unavailable',
      () async {
        final ts = TokenStorage();
        await ts.save(accessToken: 'a');
        await ts.setBiometricEnabled(true);
