...
void main() {
  ...
  test(
    'subscriber logs in and reads their own data via /me/* endpoints',
    () async {
      ...
      if (invoices.items.isNotEmpty) {
        ...
        print(
          '   e.g. ${inv.currency} ${inv.total} status=${inv.status} '
          'due=${inv.dueAt}',
        );
      }
      expect(
        invoices.count,
        greaterThan(0),
        reason: 'subscriber should see their OWN invoices without staff scopes',
      );
      ...
      print(
        '✅ /me/quota-buckets — count=${quota.count} (empty: no quota data)',
      );

      final sessions = await usage.sessions();
      final down = sessions.items.fold<int>(
        0,
        (s, e) => s + (e.outputOctets ?? 0),
      );
      final up = sessions.items.fold<int>(
        0,
        (s, e) => s + (e.inputOctets ?? 0),
      );
      print(
        '✅ /me/radius-accounting-sessions — count=${sessions.count} '
      ...
      print(
        '✅ /me/ledger — count=${ledger.items.length}'
      ...
      print(
        '✅ /me/balance — credit=${balance.creditBalance} '
        '${balance.currency}',
      );

      final topup = await billing.topupPage();
      print(
        '✅ /me/topup — balance=${topup.prepaidBalance} '
  ...
  test(
    'access token refreshes and rotates via the stored refresh token',
    () async {
      ...
      expect(
        newRefresh,
        isNot(oldRefresh),
        reason: 'refresh token must rotate',
      );
  ...
  test(
    'lists the caller\'s active sessions with one marked current',
    () async {
      await auth.login(username: _user, password: _pass, provider: 'local');
      final sessions = await auth.sessions();
      expect(sessions, isNotEmpty);
      expect(
        sessions.where((s) => s.isCurrent).length,
        greaterThanOrEqualTo(1),
      );
      print(
        '✅ /auth/me/sessions — ${sessions.length} session(s), '
